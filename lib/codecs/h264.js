    ...
    while (this.i < len) {
      ...
      switch (this.buffer[this.i]) {
        case 0:
          ...
          do {
            this.i++;
          } while (this.buffer[this.i] !== 1 && this.i < len);
...
export class H264Stream extends Stream {
  constructor() {
    ...
    this.nalByteStream.on('done', function() {
      this.trigger('done');
    });

    this.nalByteStream.on('partialdone', function() {
      this.trigger('partialdone');
    });

    this.nalByteStream.on('reset', function() {
      this.trigger('reset');
    });

    this.nalByteStream.on('endedtimeline', function() {
      this.trigger('endedtimeline');
    });
  }
   ...
   */
  skipScalingList(count, expGolombDecoder) {
    var
      lastScale = 8,
      nextScale = 8,
      j,
   ...
   * Prevention Bytes
   */
  discardEmulationPreventionBytes(data) {
    var
