...
import { mdat as _mdat, moof as _moof, initSegment as _initSegment } from './mp4-generator.js';
import { groupNalsIntoFrames, groupFramesIntoGops, extendFirstKeyFrame, generateSampleTable, concatenateNalData } from './frame-utils';
import { trimAdtsFramesByEarliestDts, prefixWithSilence, generateSampleTable as _generateSampleTable, concatenateFrameData } from './audio-frame-utils';
import { collectDtsInfo, calculateTrackBaseMediaDecodeTime, clearDtsInfo } from './track-decode-info';
import { MetadataStream, TimestampRolloverStream, TransportPacketStream, TransportParseStream, ElementaryStream, CaptionStream } from '../m2ts/m2ts.js';
import { metadataTsToSeconds } from '../utils/clock';
...
import AUDIO_PROPERTIES, { forEach } from '../constants/audio-properties.js';
import VIDEO_PROPERTIES, { forEach as _forEach } from '../constants/video-properties.js';
...
export class AudioSegmentStream extends Stream {
  ...
  flush() {
    ...
    this.track.samples = _generateSampleTable(frames);
    ...
    frameDuration = Math.ceil(ONE_SECOND_IN_TS * 1024 / this.track.samplerate);
...
export class VideoSegmentStream extends Stream {
  ...
  flush() {
    ...
    // (group-of-pictures)
    frames = groupNalsIntoFrames(this.nalUnits);
    ...
    if (!gops[0][0].keyFrame) {
      ...
      gopForFusion = this.getGopForFusion_(this.nalUnits[0], this.track);
    ...
    collectDtsInfo(this.track, gops);

    // First, we have to build the index from byte locations to
    // samples (that is, frames) in the video data
    this.track.samples = generateSampleTable(gops);

    // Concatenate the video data and construct the mdat
    mdat = _mdat(concatenateNalData(gops));
 ...
 */
class CoalesceStream extends Stream {
  ...
  flush(flushSource) {
    ...
    if (this.videoTrack || this.audioTrack) {
      ...
      initSegment = _initSegment(this.pendingTracks);
      ...
      for (i = 0; i < this.pendingMetadata.length; i++) {
        ...
        id3.cueTime = metadataTsToSeconds(
 ...
 */
export class Transmuxer extends Stream {
  constructor(options = {}) {
    super();
    ...
    this.options = options;
    this.baseMediaDecodeTime = this.options.baseMediaDecodeTime || 0;
